...
adminApiHelper.processControlNodeAutoCompleteList = function(resultJSON, resultArr) {
    var len = 0, idx = 0;
    resultJSON['peerSource'] = [];
    ...
    var peerSrcData = adminApiHelper.parseBGPNeighborResponse(resultArr[1]);
    if (peerSrcData && peerSrcData.length) {
        peerSrcCount = peerSrcData.length;
        
        for (i = 0; i < peerSrcCount; i++) {
            resultJSON['peerSource'][i] = peerSrcData[i]['peer'];
            pos = resultJSON['peerSource'][i].indexOf(':');
            if (pos != -1) {
                resultJSON['peerSource'][i] = resultJSON['peerSource'][i].slice(pos + 1);
            }
        }
    }
    ...
    var routeTable = adminApiHelper.parseBGPRoutingTableResponse(resultArr[2]);
    if (routeTable && routeTable.length) {
        len = routeTable.length;
        for (i = 0; i < len; i++) {
            resultJSON['route'][i] = routeTable[i]['routing_table_name'][0]['_'];
        }
    }
}
