    ...
    try {
        ...
        for (var i = 0; i < cnt; i++) {
            try {
                ...
                configIndex = doNodeExist(configData, moduleName, uveData[i]['name']);
                if (-1 != configIndex) {
                    resultJSON[j]['nodeStatus'] = 'Up';
                } else {
...
function getConfigNodesSummary (req, res, appData)
{
    ...
    urlLists[0] = '/analytics/config-node/*';
    if (null != addGen) {
        urlLists[1] = '/analytics/generator/*';
    }
    ...
    for (var i = 0; i < ipCnt; i++) {
        for (var j = 0; j < uveCnt; j++) {
            ...
            try {
                ...
                if (ip[0][0] == ipList[i]) {
                    if (details) {
                        collUVE[j]['QueryStats'] = qStats[i];
                    } else {
                        collUVE[j]['value']['QueryStats'] = qStats[i];
                    }
                    break;
                }
            } catch(e) {
