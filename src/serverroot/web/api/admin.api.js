...
adminapi.getControlNodesRouteComboList = function(req, res) {
    ...
    var urlLists = [];
    urlLists[0] = [url];
    url = ip + '@' + global.SANDESH_CONTROL_NODE_PORT + '@' +
                    '/Snh_BgpNeighborReq?domain=&ip_address=';
    urlLists[1] = [url]
    url = ip + '@' + global.SANDESH_CONTROL_NODE_PORT + '@' +
                    '/Snh_ShowRouteReq?routing_instance=&routing_table=' +
                    '&prefix=&start_routing_instance=&start_routing_table=&start_prefix=&count=';
    urlLists[2] = [url];
    async.map(urlLists, commonUtils.getDataFromSandeshByIPUrl(rest.getAPIServer, true), 
            function(err, results) {
        if (!err) {
            var resultJSON = {};
            adminApiHelper.processControlNodeAutoCompleteList(resultJSON, results);
            commonUtils.handleJSONResponse(null, res, resultJSON);
        } else {
